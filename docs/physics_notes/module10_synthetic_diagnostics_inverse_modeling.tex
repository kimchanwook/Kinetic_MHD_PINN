...
\rhead{Module 10 Physics Note}

\begin{document}
\DocTitle{Module 10: Synthetic Diagnostics and Physics-Informed Inverse Modeling}{Physics-Note Scaffold for \texttt{Kinetic\_MHD\_PINN}}{Module boundary, mathematical anchor, interfaces, verification obligations, and surrogate role}

\begin{overviewbox}
\textbf{Purpose of this scaffold.} Generate diagnostic signals from simulated states and reconstruct energetic-particle distributions, mode amplitudes, or profile changes from incomplete observations. This document establishes the module boundary before a full derivation or implementation is written. Statements labeled as planned work are not claims of a completed solver.
\end{overviewbox}
...
\section{Scope}
\textbf{Module purpose.} Generate diagnostic signals from simulated states and reconstruct energetic-particle distributions, mode amplitudes, or profile changes from incomplete observations.

\textbf{Upstream dependency.} Modules 3--8.

\textbf{Primary input contract.} coupled plasma state, diagnostic geometry, response functions, noise model, and observation cadence.

\textbf{Primary output contract.} synthetic magnetic, neutron, fast-ion-loss, and heat-load signals plus reconstructed latent states and uncertainties.
...
Physics-informed inverse model combining diagnostic likelihoods with transport, eigenmode, and conservation residuals.
