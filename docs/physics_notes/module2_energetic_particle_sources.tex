...
\begin{document}
\DocTitle{Module 2: Energetic-Particle Sources and Distribution Functions}{Physics-Note Scaffold for \texttt{Kinetic\_MHD\_PINN}}{Module boundary, mathematical anchor, interfaces, verification obligations, and surrogate role}

\begin{overviewbox}
\textbf{Purpose of this scaffold.} Construct alpha-particle and neutral-beam source terms and reduced energetic-particle distributions in energy, pitch, radius, and time. This document establishes the module boundary before a full derivation or implementation is written. Statements labeled as planned work are not claims of a completed solver.
\end{overviewbox}
...
\section{Scope}
\textbf{Module purpose.} Construct alpha-particle and neutral-beam source terms and reduced energetic-particle distributions in energy, pitch, radius, and time.

\textbf{Upstream dependency.} Module 1 and prescribed thermal-plasma profiles.

\textbf{Primary input contract.} equilibrium geometry, $n_D$, $n_T$, $T_i$, beam parameters, and source normalization.

\textbf{Primary output contract.} $S_f(\psi,E,\lambda,t)$, $f_f(\psi,E,\lambda,t)$, birth power, and moments of the fast-ion population.
...
Physics-informed or operator-learning surrogate for reduced Fokker--Planck evolution and source-to-distribution maps.
