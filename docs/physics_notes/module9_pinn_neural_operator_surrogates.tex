...
\rhead{Module 9 Physics Note}

\begin{document}
\DocTitle{Module 9: PINN and Neural-Operator Surrogates}{Physics-Note Scaffold for \texttt{Kinetic\_MHD\_PINN}}{Module boundary, mathematical anchor, interfaces, verification obligations, and surrogate role}

\begin{overviewbox}
\textbf{Purpose of this scaffold.} Develop and compare PINNs, causal PINNs, neural operators, and structure-preserving surrogates for selected expensive forward and inverse maps. This document establishes the module boundary before a full derivation or implementation is written. Statements labeled as planned work are not claims of a completed solver.
\end{overviewbox}
...
\section{Scope}
\textbf{Module purpose.} Develop and compare PINNs, causal PINNs, neural operators, and structure-preserving surrogates for selected expensive forward and inverse maps.

\textbf{Upstream dependency.} Training products from Modules 1--8.

\textbf{Primary input contract.} high-fidelity or reduced-model datasets, governing residuals, boundary conditions, conservation laws, and uncertainty targets.

\textbf{Primary output contract.} trained surrogates, error budgets, calibration metrics, speedups, and admissibility diagnostics.
...
This module is the cross-cutting surrogate layer and defines model-selection criteria for each physical subproblem.
